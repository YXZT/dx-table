...
const { localColums } = init();

const hiddenColumsOptions = computed(() => {
  const arr = localColums.value
...
<template>
  <div class="search">
    <div class="search-area">
      <div
        v-for="item in ellipsisTableSearchColumns"
        :key="item.prop"
        class="item-box"
      >
        <n-form-item
        ...
        >
          <SearchFormItem
            :column="item"
            :search-param="searchData"
            @value-change="refreshTable"
          />
        </n-form-item>
      </div>
    ...
    <div class="search-area" v-if="!collapsed">
      <div v-for="item in tableSearchColumns" :key="item.prop" class="item-box">
        ...
        <n-icon-wrapper
          ...
          color="#ccc"
          :border-radius="18"
          @click="removeSearchColumns(item)"
        >
          <n-icon :size="18" :component="CloseSharp" />
        </n-icon-wrapper>
      </div>
      <n-popselect
        :options="hiddenColumsOptions"
        size="medium"
        scrollable
        :on-update:value="showSearchColumns"
